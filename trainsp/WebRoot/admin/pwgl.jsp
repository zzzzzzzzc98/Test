...
<link href="/trainsp/images/skin.css" rel="stylesheet" type="text/css" />
  ...
  <body>
   <%
String url="/trainsp/admin/pwgl.jsp?1=1";
...
sql+=" order by id desc ";
 %>
     <form action="/trainsp/admin/pwgl.jsp?f=f" method="post" name="f1">
  ...
  <tr>
    ...
    <td valign="top" background="/trainsp/images/content-bg.gif"><table width="100%" height="31" border="0" cellpadding="0" cellspacing="0" class="left_topbg" id="table2">
      <tr>
        <td height="31"><div class="titlebt">票务管理</div></td>
      </tr>
    </table></td>
  ...
  <tr>
    ...
    <table width="98%" align="center" border="0" cellpadding="3" cellspacing="1" bgcolor="#CBD8AC" class="left_txt">
      	<tr >
      		...
      		<td align="center" bgcolor="#f2f2f2">始发</td>
      		<td align="center" bgcolor="#f2f2f2">到达</td>
      		<td align="center" bgcolor="#f2f2f2">余票情况</td>
      		<td align="center" bgcolor="#f2f2f2">运行时间</td>
      		<td align="center" bgcolor="#f2f2f2">车票日期</td>
  ...
  PageManager pageManager = PageManager.getPage(url,10, request);
   ...
   %>
        <tr bgcolor="#FFFFFF">
      		...
      		<td align="center"><%=map.get("saddr") %>(<%=map.get("stime") %>)</td>
      		<td align="center"><%=map.get("eaddr") %>(<%=map.get("etime") %>)</td>
      		<td align="center">特等座：余<%=map.get("tdzsl") %>张(<%=map.get("tdzjg") %>元)&nbsp;|&nbsp;一等座：余<%=map.get("ydzsl") %>张(<%=map.get("ydzjg") %>元)&nbsp;|&nbsp;二等座：余<%=map.get("edzsl") %>张(<%=map.get("edzjg") %>元)</td>
      		<td align="center"><%=map.get("yxsj") %> 小时</td>
      		<td align="center"><%=map.get("sj") %></td>
      		<td align="center"><a href="javascript:up(<%=map.get("id") %>)">编辑</a>&nbsp;&nbsp;|&nbsp;&nbsp;
      		<a href="/trainsp/admin/pwgl.jsp?did=<%=map.get("id") %>">删除</a></td>
      	</tr>
      	<%} %>
